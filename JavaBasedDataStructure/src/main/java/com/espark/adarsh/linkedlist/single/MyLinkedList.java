...
public class MyLinkedList<T> {
    ...
    public void sortListNodes() {
        Node<T> newStart = start;
        boolean flag = true;
        for (Node<T> list = start; list.getNext().getNext() != null; list = list.getNext(), flag = false) {

            for (Node<T> listNext = list.getNext(); listNext.getNext() != null; listNext = listNext.getNext()) {
                final int result = this.comparator.compare(list.getData(), listNext.getData());
                if (result == 1 ) {

                    Node<T> firstNode = list;
                    Node<T> middleNode = firstNode.getNext();
                    Node<T> nextNode = middleNode.getNext();
